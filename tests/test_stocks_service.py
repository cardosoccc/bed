...
class TestListStocks:
    async def test_empty(self, db_session):
        result = await service.list_stocks(db_session)
        assert result == []

    async def test_returns_ordered(self, db_session):
        db_session.add(Stock(ticker="petr4", price=30.0))
        db_session.add(Stock(ticker="bbas3", price=25.0))
...
class TestGetStock:
    async def test_found(self, db_session):
        db_session.add(Stock(ticker="vale3", price=60.0))
        await db_session.commit()

        result = await service.get_stock(db_session, "vale3")
        assert result is not None
        assert result.price == 60.0

    async def test_not_found(self, db_session):
        result = await service.get_stock(db_session, "nope")
        assert result is None


class TestAddStock:
    async def test_add(self, db_session):
        result = await service.add_stock(db_session, "itub4")
        assert result.ticker == "itub4"
        assert result.price == 0

        fetched = await service.get_stock(db_session, "itub4")
        assert fetched is not None


class TestRemoveStock:
    async def test_remove_existing(self, db_session):
        db_session.add(Stock(ticker="mglu3", price=5.0))
        await db_session.commit()

        assert await service.remove_stock(db_session, "mglu3") is True
        assert await service.get_stock(db_session, "mglu3") is None

    async def test_remove_missing(self, db_session):
        assert await service.remove_stock(db_session, "nope") is False
...
class TestUpdatePrices:
    async def test_updates_stock_and_asset(self, db_session, monkeypatch):
        db_session.add(Stock(ticker="vale3", price=0))
        db_session.add(Asset(
            name="petr4", asset_class=AssetClass.equity, asset_type=AssetType.stock,
            quantity=100, initial_value=3000, current_value=3000,
        ))
        ...
        stock = await service.get_stock(db_session, "vale3")
        assert stock.price == 50.0

        # petr4 should also have a stock entry now
        petr_stock = await service.get_stock(db_session, "petr4")
        assert petr_stock is not None
        assert petr_stock.price == 50.0
